...
function canCellsMerge(cell1, cell2, now) {
    const timeSinceSplit1 = now - (cell1.splitTime || 0);
    const timeSinceSplit2 = now - (cell2.splitTime || 0);
    return timeSinceSplit1 > MERGE_COOLDOWN && timeSinceSplit2 > MERGE_COOLDOWN;
}

function applyMergingForces(cell1, cell2, distance, canMerge) {
...
function mergeCellsAtIndices(indices) {
    const cells = indices.map(index => gameState.playerCells[index]);
...
function updateCellMerging() {
    ...
    for (let i = 0; i < gameState.playerCells.length; i++) {
        ...
        for (let j = i + 1; j < gameState.playerCells.length; j++) {
            ...
            const canMerge = canCellsMerge(cell1, cell2, now);

            if (distance < minMergeDistance && canMerge) {
                if (distance < (cell1Size + cell2Size) * 0.5) {
                ...
                applyMergingForces(cell1, cell2, distance, canMerge);
            }
        }
    }
    ...
    if (cellsToMerge.length > 0) {
        ...
        for (let i = 1; i < uniqueIndices.length; i++) {
            if (uniqueIndices[i - 1] - uniqueIndices[i] === 1) {
                currentGroup.push(uniqueIndices[i]);
            } else {
                groups.push(currentGroup);
                currentGroup = [uniqueIndices[i]];
            }
        }
        groups.push(currentGroup);

        groups.forEach(group => mergeCellsAtIndices(group));
    }
}
