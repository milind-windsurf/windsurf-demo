...
function processCellInteraction(cell1, cell2, i, j, cellsToMerge, now) {
    ...
    if (distance < minMergeDistance && canMerge) {
        if (distance < minDistance * 0.5) {
            ...
            applyAttractionForce(cell1, cell2, distance, MERGE_FORCE);
        }
    } else {
        if (distance < minDistance) {
            applyRepulsionForce(cell1, cell2, distance, minDistance);
        }
        
        if (distance > minDistance) {
            const force = canMerge ? MERGE_FORCE : MERGE_START_FORCE;
            applyAttractionForce(cell1, cell2, distance, force);
        }
    }
}

function applyAttractionForce(cell1, cell2, distance, force) {
...
function updateCellMerging() {
    ...
    for (let i = 0; i < gameState.playerCells.length; i++) {
        ...
        for (let j = i + 1; j < gameState.playerCells.length; j++) {
            ...
            processCellInteraction(cell1, cell2, i, j, cellsToMerge, now);
        }
    }
